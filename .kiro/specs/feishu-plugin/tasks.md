...
- [x] 1. 项目脚手架和类型定义
  - [x] 1.1 创建 `xgw/plugins/feishu/package.json`、`tsconfig.json`、`tsup.config.ts`、`vitest.config.ts`
    - 参考 TUI plugin 的配置结构
    - 添加 `@larksuiteoapi/node-sdk` 依赖
    - 添加 `fast-check` 和 `vitest` 开发依赖
    - _Requirements: 9.1_
  - [x] 1.2 在 `src/index.ts` 中定义本地 xgw 类型副本（Message、SendParams、HealthResult、PairResult、ChannelConfig、Attachment）和 FeishuMessageEvent 类型
    - 与 TUI plugin 保持一致的类型定义模式
    - 添加 FeishuPluginConfig 接口
    - _Requirements: 9.1, 9.2, 9.3, 9.4_

- [x] 2. 实现消息内容解析模块 (`src/event-handler.ts`)
  - [x] 2.1 实现 `parseMessageContent` 函数
    - 处理 text、post、image、file、unsupported 类型
    - 实现 `parsePostContent` 递归提取 post 富文本中的文本节点
    - JSON 解析失败返回空字符串
    - _Requirements: 3.1, 3.2, 3.3, 3.4, 3.5, 3.6_
  - [x]* 2.2 编写消息解析属性测试 (`vitest/pbt/message-parsing.pbt.test.ts`)
  ...
  - [x]* 2.6 编写 session 路由属性测试 (`vitest/pbt/session-routing.pbt.test.ts`)
    - **Property 7: Session ID routing**
    - **Validates: Requirements 5.1, 5.2**

- [x] 3. Checkpoint — 确保消息解析模块测试通过
  - 运行 `vitest run`，确保所有测试通过，有问题请询问用户。

- [x] 4. 实现飞书 SDK 封装 (`src/client.ts`)
  - [x] 4.1 实现 `createClient`、`createWSClient`、`createDispatcher`、`validateCredentials` 函数
...
- [x] 5. 实现 Streaming 缓冲模块 (`src/streaming.ts`)
  - [x] 5.1 实现 `StreamingBuffer` 类
    - handleChunk: 首次发送占位消息，后续累积并按 coalesceMs 合并编辑
    - handleEnd: 最终编辑并清理 session
    - clear: 清理所有 session 和定时器
    - 编辑失败时 fallback 到发送新消息
    - _Requirements: 7.1, 7.2, 7.3, 7.4, 7.5_
  - [x]* 5.2 编写 streaming buffer 属性测试 (`vitest/pbt/streaming.pbt.test.ts`)
    - **Property 8: Streaming buffer accumulation**
    - **Validates: Requirements 7.2**
  - [x]* 5.3 编写 streaming buffer 单元测试 (`vitest/unit/streaming.test.ts`)
    - 测试首次 chunk 发送占位消息
    - 测试 end 清理 session
    - 测试编辑失败 fallback
    - 测试 coalesceMs 默认值
...
- [x] 6. Checkpoint — 确保 streaming 模块测试通过
  - 运行 `vitest run`，确保所有测试通过，有问题请询问用户。

- [x] 7. 实现 FeishuPlugin 主类 (`src/index.ts`)
  - [x] 7.1 实现 FeishuPlugin 类，组装 client、event-handler、streaming 模块
  ...
  - [x]* 7.2 编写 FeishuPlugin 单元测试 (`vitest/unit/feishu-plugin.test.ts`)
